...
export default function GatewayCard({ gateway, formData }: Props) {
  const [showDetails, setShowDetails] = useState(false);

  // Get top 6 currencies with user-selected ones prioritized
  const getDisplayCurrencies = () => {
    const userCurrencyCodes = formData.currencies.map(curr => 
      curr.split('(')[1]?.replace(')', '') || curr
    );
    
    const prioritized = gateway.supportedCurrencies.filter(curr => 
      userCurrencyCodes.includes(curr)
    );
    
    const remaining = gateway.supportedCurrencies.filter(curr => 
      !userCurrencyCodes.includes(curr)
    );
  ...
  const getDisplayPaymentMethods = () => {
    const prioritized = gateway.supportedPaymentMethods.filter(method => 
      formData.paymentMethods.includes(method)
    );
    
    const remaining = gateway.supportedPaymentMethods.filter(method => 
      !formData.paymentMethods.includes(method)
    );
